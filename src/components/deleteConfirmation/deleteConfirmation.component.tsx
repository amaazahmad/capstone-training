     ...
     return (
          <div className={`${theme ? " bg-dark-gray-text-color" : "bg-white"} border-solid border-4 border-green-text-color p-8 font-lexend-deca flex flex-col items-center`}>
               ...
               <div className=' flex justify-evenly mt-6 flex-row w-full'>
                    <button onClick={() => { setDeletePopup(false) }} className=' h-14 w-20 md:w-32 m-2  text-white outline-none bg-secondary-text-color rounded-xl text-l md:text-xl' >Cancel</button>
                    <button onClick={() => { handleDelete(blogID) }} className=' h-14 w-20 md:w-32 m-2 text-white outline-none bg-red-600 rounded-xl text-l md:text-xl'>{loaderVisible ? <Bars visible={true} height="26" width="30" color="white" wrapperStyle={{ justifyContent: 'center' }} /> : "Delete"}</button>
               </div>
          </div>
     )
}
